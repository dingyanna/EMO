...
def distance(ys_pred, ys): 
    return torch.linalg.norm(ys_pred - ys, dim=2)
...
class ReluNNRegression(Task):
    def __init__(
        ...
        self.act_funcs = [torch.nn.ReLU(), torch.nn.ReLU(), torch.nn.Tanh(), torch.nn.Sigmoid()]
        if mode == 'relu':
    ...
    def evaluate(self, xs_b):
        ...
        activ = self.act_funcs[0](xs_b @ W_init) * math.sqrt(2 / self.hidden_layer_size)
        layer_activations = [activ]
        for i in range(self.n_layers-2):
            activ = self.act_funcs[i+1](activ @ Ws[i]) * math.sqrt(2 / self.hidden_layer_size)
